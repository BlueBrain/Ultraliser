...
AppOptions* parseArguments(const int& argc , const char** argv)
{
    std::unique_ptr< AppArguments > args = std::make_unique <AppArguments>(argc, argv,
              "This tool maps the input mesh to the target mesh.");

    args->addInputMeshArguments();
    args->addTargetMeshArguments();
...
void run(int argc , const char** argv)
{
    ...
    // Load the input mesh
    auto targetMesh = new Mesh(options->targetMeshPath);

    // Map the meshes
    mapMeshes(targetMesh, inputMesh);

    // Export the MC mesh
    if (options->exportOBJ || options->exportPLY || options->exportOFF || options->exportSTL)
        inputMesh->exportMesh(options->meshPrefix + MAPPED_SUFFIX,
