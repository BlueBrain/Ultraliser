...
AppOptions* parseArguments(const int& argc , const char** argv)
{
    ...
    std::unique_ptr< AppArguments > args = std::make_unique <AppArguments>(argc, argv,
              "This tool analyses the structure of an input vascular morphology "
              "and writes the results to text files where we can plot the distributions");
...
void run(int argc , const char** argv)
{
    ...
    vessMorphology->printStats(options->prefix, &outputDirectory);
    vessMorphology->printDistributions(&outputDirectory);
}
