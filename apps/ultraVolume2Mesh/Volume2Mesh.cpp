...
Options* parseArguments(const int& argc , const char** argv)
{
    ...
    Ultraliser::Argument inputVolume(
                "--volume",
                ARGUMENT_TYPE::STRING,
                "The full path to the volume.",
                ARGUMENT_PRESENCE::MANDATORY);
    args->addArgument(&inputVolume);
    ...
    /// Get all the options
    // Input / output
    options->inputVolume = args->getStringValue(&inputVolume);
...
void run(int argc , const char** argv)
{
    ...
    if (options->writeHistogram)
    {
        ...
        Ultraliser::File::writeIntegerDistributionToFile(path, histogram);
