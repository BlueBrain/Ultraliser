...
#define DEG_TO_RADS(deg) (deg * M_PI) / 180.f

namespace scr
{
  Camera::Camera(const float nearPlane, const float farPlane, const float fov)
   : _near(nearPlane)
   , _far(farPlane)
   , _fov(fov)
   , _aspectRatio(1.f)
  ...
  void Camera::onScreenResize(const uint32_t width, const uint32_t height)
  {
  ...
  void Camera::onZoom(const float val)
  {
    _fov += val;
  ...
  void Camera::update()
  {
    ...
    {
      _dirty = false;
      /*const glm::mat4 yaw = glm::rotate<float>(_rotation.x, glm::vec3(0.f, 1.f, 0.f));
      const glm::mat4 pitch = glm::rotate<float>(_rotation.y, glm::vec3(1.f, 0.f, 0.f));*/
  ...
  {
    const float rads = DEG_TO_RADS(_fov);
    const float tanRads = tan(rads);
    _projectionMatrix = glm::mat4(0.f);
