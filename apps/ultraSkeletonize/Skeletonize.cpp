...
//                              options->exportOFF, options->exportSTL);
    ...
    // Difference
    auto diffVolume = subtractVolume(solidVolume, surfaceVolume);
    diffVolume->project(prefix + "_diff",
                        options->projectXY, options->projectXZ, options->projectZY,
                        options->projectColorCoded);
    ...
    {
        auto nextShell = getNextShell(surfaceVolume, diffVolume);


        nextShell->project(prefix + "_next_" + std::to_string(i),
                           options->projectXY, options->projectXZ, options->projectZY,
                           options->projectColorCoded);

        diffVolume = subtractVolume(diffVolume, surfaceVolume);
