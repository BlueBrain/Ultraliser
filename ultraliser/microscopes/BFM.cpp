...
std::string createConfigBrightFieldMicroscopeXY(const std::string configFile,
                                                const PBRTParameters params)
{
    ...
    for (size_t i = 0; i < inputConfig.size(); i++)
    {
        ...
        stream << params.lensRadius;
        String::replaceSubstring(line, "LENS_RADIUS", stream.str());
        CLEAR_STREAM;
...
std::string createConfigBrightFieldMicroscopeZY(const std::string configFile,
                                                const PBRTParameters params)
{
