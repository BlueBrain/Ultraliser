...
AstrocyteMorphology::AstrocyteMorphology(NeuronSWCSamples &swcSamples)
{
...
void AstrocyteMorphology::_constructMorphologyFromSWC(const NeuronSWCSamples& swcSamples)
{
    ...
    _somaCenter = Vector3f(0.0f);
    _somaMeanRadius = 0.0f;
    _somaMinRadius = std::numeric_limits<float>::max();
    _somaMaxRadius = std::numeric_limits<float>::lowest();
...
AstrocyteMorphology* readAstrocyteMorphology(std::string& morphologyPath)
{
     ...
     else if (String::subStringFound(morphologyPath, std::string(".swc")) ||
